 ...
 *          reformated Scanco header information to ease readability
 *          commented Michael's usageReporter to be able to test everything within my programming environment
 *              @Michael: you can easily find the two lines -> search for "//KH"
 *          @Michael: please check two comments beginning with "//KHK"
 *          I had to refactor the name to include an underscore in order to see the plugin from within ImageJ
 *          @Michael: how do you use the plugin without the underscore and and plugin.config file?
 *          
 * 
...
//KH import org.doube.util.UsageReporter;
...
public class ISQ_Reader implements PlugIn {
	...
	public void run(String arg) {
		...
		// Open the file
		try {
			ImagePlus imp = openScancoISQ(path, downsample, startX, startY, endX, endY, startZ, nSlices);
			imp.show();
			// KH UsageReporter.reportEvent(this).send();
