...
public class Import_Clip_ISQ6 implements PlugIn {
	...
	public void run(String arg) {
		...
		fi.pixelWidth = el_size_mm_x;
		if (debug)
			System.out.println("checkpoint3a");

		fi.pixelHeight = el_size_mm_y;
		fi.pixelDepth = el_size_mm_z;
		...
		System.out.println(
				"Source code line: 252 - Message: Pixel width x: " + fi.pixelWidth + " ; y: " + fi.pixelHeight);
	...
	void getRoiCoordinates() {
		...
		// ich habe Abfrage ergänzt of fi.offset oder fi.longOffset verwendet werden
		// soll
		...
		if (nFirstSlice > 0) {
			...
			// bei einem "int"-Overflow - also Zahl die grÃ¶ÃŸer als Int ist das Ergebnis
			// negativ.
			// das muss ich auch abfragen.
			// war Stand for 16.2.08: if (fi.offset + (nFirstSlice*fi.width*fi.height*2)
			// <=2147483647){
			...
			for (int i = 1; i <= fi.nImages; i++) { // Obsolet comment: I reduce the no of slices by 1 to avoid a
				...
				if (scale4096 == true) {
					...
					for (int s = 0; s < widthROI * heightROI; s++) {
						...
						// wenn ich nun aber genau so "skalieren" will, wie mit der
						// "Koller-Import-Routine"
						// habe ich das Problem, dass die Werte nur positiv sind und um die 32768 zu
						// hoch sind
						// ich behebe das hier, indem ich beim Umwandeln in Float das Ganze rückgängig
						// mache
						// langfristig, muss ich mal die ganze Routine von Grund auf selbst und neu
						// schreiben.
					...
					if (downsample == true) {
						...
						if (i % 2 > 0) {
							...
							for (int s = 0; s < heightStack * widthStack; s++) {
								...
								temp3 = (temp1 + temp2) / 2;
								// System.out.println("temp1-temp2: "+temp1+"temp2 " +temp2+ "stack: "+
								// downsampledPixels32_temp[s] + "temp3 "+ temp3);
								downsampledPixels32_av[s] = temp3;
							}

							stack.addSlice(
									"microCT-Import_by_KH_w_" + widthROI / 2 + "_h_" + heightROI / 2 + "_slice." + i,
									downsampledPixels32_av);
						}
					} else {
						// System.out.println("Instead of downsample loop ... ELSE loop");
						stack.addSlice("microCT-Import_by_KH_w_" + widthROI + "_h_" + heightROI + "_slice." + i,
								pixels32);
					}
				} else {
					...
					if (downsample == true) {
						...
						if (i % 2 > 0) {
							...
							if (eightBitOnly == true) {
								stack.addSlice(
										"microCT-Import_by_KH_w_" + widthStack + "_h_" + heightStack + "_slice." + i,
										pixels8);
							} else {
								stack.addSlice(
										"microCT-Import_by_KH_w_" + widthStack + "_h_" + heightStack + "_slice." + i,
						...
						// *** shortTo8bit ende
						// System.out.println("Instead of downsample loop ... ELSE loop - just before
						// add stack");
	...
	/*
	 * void setCalibration(ImagePlus imp) { // A signed 16-bit image in ImageJ is
	 * represented by an unsigned image // with 32768 added and a calibration
	 * function (y=-32768+x) that subtracts 32768.
	 * 
	 * /* KH: 4.3.08 - temporarily deactivated: now I have the same values for the
	 * downsampled and the original short images if I would subtract 32768 ... then
	 * I would have the right values even compared with the float images (after
	 * division by 4096)
	 * 
	 * 
	 * if (fi.fileType==FileInfo.GRAY16_SIGNED) { if (IJ.debugMode)
	 * IJ.log("16-bit signed"); double[] coeff = new double[2]; coeff[0] = -32768.0;
	 * coeff[1] = 1.0;
