...
public class KHKs_simple_Werth_rek_FileReader implements PlugIn {
	...
	public void run(String arg) {
		...
		try {
			...
			/*
			 * tmpInt = (p.read() + p.read() * 256 + p.read() * 65536 + p.read()
			 * *256*65536); el_size_mm_x = tmpInt/xdimension; tmpInt = (p.read() + p.read()
			 * * 256 + p.read() * 65536 + p.read() *256*65536); el_size_mm_y =
			 * tmpInt/ydimension; tmpInt = (p.read() + p.read() * 256 + p.read() * 65536 +
			 * p.read() *256*65536); el_size_mm_z = tmpInt/zdimension;
			 * 
			 * el_size_mm_x = el_size_mm_x/1000; el_size_mm_y = el_size_mm_y/1000;
